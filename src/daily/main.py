...
The lifespan context manager wires the APScheduler briefing pipeline cron job:
  - On startup: parse briefing_schedule_time from Settings, call setup_scheduler,
    then scheduler.start(). Scheduler runs within the same asyncio event loop.
...
If the user has saved a config via `daily config set briefing.schedule_time`, it takes
effect on next app restart (the cron job reads DB config at startup in a future
enhancement — for M1 single-user, env/settings default is sufficient).
"""
