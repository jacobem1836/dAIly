...
result: pass
note: "Verified on device 2026-05-09."
...
result: fail
note: "grey → yellow → green (Listening) works. Blue 'Speaking' state does not appear when AI responds. Indicator stays green while agent is speaking. 2026-05-09."
...
- **Speaking state not surfaced (test 6):** ConnectionIndicator stays green while agent speaks. The agent is responding (confirmed via worker logs — `speech playout started`) but the iOS client is not receiving or acting on the speaking state event from LiveKit. Needs investigation in VoiceSession.swift / ConnectionIndicator state mapping.
