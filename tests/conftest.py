...
# ---------------------------------------------------------------------------
# Phase 21 — Wave 0 shared fixtures
# ---------------------------------------------------------------------------


@pytest.fixture
async def mock_redis():
    """Async Redis fixture backed by fakeredis (in-memory, no real Redis needed).

    Used by Plan 03 (integrations router) to test OAuth state CSRF validation.
    Provides a ``redis.asyncio``-compatible interface via ``fakeredis.aioredis``.
    """
...
def auth_headers() -> dict[str, str]:
    """Return ``Authorization: Bearer <token>`` headers for test user ID 100.

    Uses a deterministic JWT_SECRET ("x" * 32) so callers do not need to
    configure environment variables themselves.  The monkeypatch approach
    used in individual test files works equally well — this fixture is a
    convenience for plans that don't need a full DB setup.
    """
    import os
