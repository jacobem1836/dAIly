...
# ---------------------------------------------------------------------------
# Phase 9 INTEL-02: Memory preamble injection tests
# ---------------------------------------------------------------------------


class TestNarratorMemoryPreamble:
    def test_narrator_prompt_includes_memory_preamble(self):
        """build_narrator_system_prompt with user_memories injects MEMORY_PREAMBLE."""
        from daily.briefing.narrator import build_narrator_system_prompt

        result = build_narrator_system_prompt(
            preferences=None,
            user_memories=["User travels to Sydney", "User prefers concise emails"],
        )
        assert "User travels to Sydney" in result
        assert "User memories" in result  # MEMORY_PREAMBLE header phrase

    def test_narrator_prompt_no_memory_preamble_when_empty(self):
        """build_narrator_system_prompt with user_memories=[] does NOT include memory preamble."""
        from daily.briefing.narrator import MEMORY_PREAMBLE, build_narrator_system_prompt

        result_empty = build_narrator_system_prompt(user_memories=[])
        result_none = build_narrator_system_prompt(user_memories=None)

        # Extract the header phrase from MEMORY_PREAMBLE
        assert "User memories" not in result_empty
        assert "User memories" not in result_none

    def test_narrator_prompt_memory_before_preferences(self):
        """Memory preamble appears before preferences preamble in the output."""
        from daily.briefing.narrator import build_narrator_system_prompt

        prefs = UserPreferences(tone="casual")
        result = build_narrator_system_prompt(
            preferences=prefs,
            user_memories=["User travels to Sydney"],
        )

        memories_pos = result.index("User memories")
        prefs_pos = result.index("User preferences")
        assert memories_pos < prefs_pos, (
            "Memory preamble should appear before preferences preamble"
        )

    def test_narrator_memory_preamble_constant_defined(self):
        """MEMORY_PREAMBLE constant is defined in the narrator module."""
        from daily.briefing import narrator
        assert hasattr(narrator, "MEMORY_PREAMBLE")

    def test_narrator_backward_compat_no_memories_param(self):
        """build_narrator_system_prompt called with only preferences still works."""
        from daily.briefing.narrator import NARRATOR_SYSTEM_PROMPT, build_narrator_system_prompt

        # Original signature: build_narrator_system_prompt(preferences=None)
        result = build_narrator_system_prompt(UserPreferences())
        assert NARRATOR_SYSTEM_PROMPT in result
